...
void pipes(char ***input, int input_num) {
    ...
    int output_fd = 1;
    while(*input != NULL){
          // Check for file redirection symbols in input arguments
        for(int i = 0; (*input)[i] != NULL; i++){
            if(strcmp((*input)[i], "<") == 0){
                // Open input file and replace standard input with input file descriptor
                input_fd = open((*input)[i+1], O_RDONLY);
                if(input_fd < 0){
                ...
                dup2(input_fd, 0);
                close(input_fd);
                // Remove input redirection arguments from input list
                for(int j = i; (*input)[j] != NULL; j++){
                    (*input)[j] = (*input)[j+2];
                }
                //exec(*input[0], *input);
                ...
                // Open output file and replace standard output with output file descriptor
                output_fd = creat((*input)[i+1], 0644);
                if(output_fd < 0){
                ...
                dup2(output_fd, 1);
                close(output_fd);
                // Remove output redirection arguments from input list
                for(int j = i; (*input)[j] != NULL; j++){
                    (*input)[j] = (*input)[j+2];
                }
                //exec(*input[0], *input);
...
char*** pipe_parse(char** input, int arg_count) {
    ...
    for (int i = 0; i < arg_count; i++) {
        if (!strcmp(input[i], "|")) {
            ...
            index++;
            j = 0;
            new_input[index][j] = strdup(input[i]); // save "<"
            j++;
            new_input[index][j] = strdup(input_file); // save input file name
            j++;
            new_input[index][j] = NULL;
            close(fd);
            i++;
            ...
            new_input[index][j] = strdup(input[i]); // save ">"
            j++;
            new_input[index][j] = strdup(output_file); // save output file name
            j++;
            new_input[index][j] = NULL;
            close(fd);
            i++;
